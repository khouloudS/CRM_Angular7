...
export class EmbryoService {
   ...
   constructor(private http:HttpClient,
      ...
      //this.db.object("products").valueChanges().subscribe(res => {this.setCartItemDefaultValue(res['gadgets'][0])});
   ...
   public setCartItemDefaultValue(setCartItemDefaultValue) {
      ...
      let found = products.some(function (el, index, array) {
  ...
  {
    let users : any;
    users = JSON.parse(localStorage.getItem("cart_item")) || [];
    console.log(users);
    let found = users.some(function (el, index) {
      if(el.email == user.email && el.password == user.passord){
        return  true;
      }
    });
    localStorage.setItem("cart_item", JSON.stringify(users));
  }
}
